...
def uncertainty_reward(completions, strict_entailment=True, **kwargs):
    ...
    semantic_set_ids, semantic_set_counts = get_semantic_ids(answer_contents, strict_entailment=strict_entailment)
    ...
    for confidence_word, semantic_set_count in zip(confidence_words, semantic_set_counts):
        ...
        if semantic_set_count < (len(contents) / 2) : # 暂时更改， for llama_pararel_batch10_strict_answer_alldata_6th.yaml 
...
def accuracy_reward(completions, solution, strict_entailment=True, **kwargs):
    """检查完成是否与基准答案相同的奖励函数"""
    contents = [completion[0]["content"] for completion in completions]
    rewards = []
...
def format_reward(completions, **kwargs):
    ...
    # pattern2 = r"^<answer>.*?</answer>.*?<confidence>.*?</confidence>$"
    ...
    # matches2 = [0.5 if re.match(pattern2, content, re.DOTALL) else 0.0 for content in completion_contents]
    # sum_score = [score1+ score2 for score1, score2 in zip(matches, matches2)]
    return matches
