...
import pandas as pd
# 解析命令行参数
parser = argparse.ArgumentParser(description="测试模型在falseqa数据集上的表现")
parser.add_argument("--model_path", type=str, default="results/qwen/pararel_batch10_strict_answer_origin_data", help="模型路径")
parser.add_argument("--data_path", type=str, default='dataset/falseqa_test.csv', help="数据集路径")
parser.add_argument("--output_path", type=str, default='evaluate/qwen/falseqa/pararel_batch10_strict_answer_origin_data_falseqa_results2.json', help="结果保存路径")
...
# 加载数据集
def load_falseqa_data(data_path):
    data = pd.read_csv(data_path)
    # the head is question, answer, label
    dataset = []
    for i in range(len(data)):
        ...
        if label == 1:
            dataset.append({'question': question, 'answer': answer, 'label': label})
    return dataset

dataset = load_falseqa_data(args.data_path)
...
Answer: <answer> Balck vulture belongs to Coragyps. </answer>
<confidence> unsure </confidence>
