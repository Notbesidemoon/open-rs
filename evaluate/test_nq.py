...
import datasets
import re
#  python evaluate/test_nq.py --model_path results/pararel --output_path evaluate/qwen/nq/pararel_trained_rft_qwen_nq_results.json --gpu_id 0
# 解析命令行参数
parser = argparse.ArgumentParser(description="测试模型在nqopen数据集上的表现")
parser.add_argument("--model_path", type=str, default="results/llama3/pararel_batch10_53_strict_answer_se_1", help="模型路径")
parser.add_argument("--data_path", type=str, default='nq_open', help="数据集路径")
parser.add_argument("--output_path", type=str, default='evaluate/llama3/nq/pararel_trained_rft_nq_results.json', help="结果保存路径")
...
# 加载数据集
dataset = datasets.load_dataset("nq_open", split="validation")
